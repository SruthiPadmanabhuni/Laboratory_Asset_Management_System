...
const IssuesTable = (data_table) => {
  ...
  const status=data_table.status;
  ...
  return (
    <div style={{ textAlign: 'left', paddingLeft: '1px', paddingTop: '30px' }}>
      <table style={{ border: "2px solid black" }}>
        <thead>
          <tr>
            ...
            {(status !== "resolved") ? (<th style={{ border: "2px solid black" }} className='p-2'>Update Status</th>) : null}
          </tr>
        </thead>
        <tbody>
          {data.map((issue) => (
